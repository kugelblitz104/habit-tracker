...
class TestPatchProject:
    ...
    async def test_patch_project_with_an_empty_body_only_stamps_updated_date(
        self, client, db_session, login_as
    ):
        ...
        project = ProjectFactory(profile=profile, name="Untouched", archived=False)
        await db_session.commit()
        assert project.updated_date is None  # so a bump is visible

        await login_as(user)

        response = await client.patch(f"/projects/{project.id}", json={})
